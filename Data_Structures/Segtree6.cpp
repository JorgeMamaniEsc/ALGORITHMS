...
struct SegTree{
    ...
    int cnt(int u,int l,int r,int x){
        ...
        if(l==r) return 1;
        int mid=(l+r)/2;
        if(tree[2*u].mx<=x) return cnt(2*u+1,mid+1,r,x);
        return cnt(2*u,l,mid,x)+(tree[u].ans-tree[2*u].ans);
    }
    ...
    void build(int u,int l,int r,vi &v){
        ...
        tree[u].ans = tree[2*u].ans + cnt(2*u + 1, mid + 1, r, tree[2*u].mx);
    }
    ...
    int qry(int u,int l,int r,int ql,int qr,int &mx){
        if(ql<=l&&r<=qr){
            int p=cnt(u,l,r,mx);
            mx=max(mx,tree[u].mx);
            return p;
        ...
        int mid=(l+r)/2;
        return qry(2*u,l,mid,ql,qr,mx)+qry(2*u+1,mid+1,r,ql,qr,mx);
    }
};
...
signed main(){
    ...
    while(tt--){
        ...
        while(q--){
            ...
            l--;r--;
            int g=0;
            cout<<st.qry(1,0,n-1,l,r,g)<<endl;
        }
    }
    return 0;
}
