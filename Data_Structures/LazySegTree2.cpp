...
signed main(){
    ...
    while(tt--){
        int n;cin>>n;
        vi next(2*n,2*n),fin(2*n,-1),ini(2*n,-1),v(2*n);
        vvi pos(2*n);
        rep(i,0,2*n,1){
            int x,idx;cin>>x>>idx;
            idx--;
        ...
        int ans=0;
        rep(i,0,2*n,1){
            if(fin[i]!=-1) ans+=fin[i]-i+1;
        }
        set<int> sts;
        rep(i,0,2*n,1){
            sts.insert(v[i]);
            st.update(1,0,2*n,i,i,sz(sts));
        }
        rep(i,0,2*n,1){
            rep(j,0,sz(pos[i])-1,1) next[pos[i][j]]=pos[i][j+1];
        }
        rep(l,0,2*n,1){
            int r=fin[l];
            if(r!=-1){
                ans-=st.qry(1,0,2*n,r,r);
            }
            st.update(1,0,2*n,l,next[l]-1,-1);
        }
        cout<<ans<<endl;
    }
    return 0;
