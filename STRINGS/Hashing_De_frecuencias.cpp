...
void mikunakano(){
    int n,m;cin>>n>>m;
    vvi ady(n);
    vi h1(n+2,0),h2(n+2,0),p1(n+2,1),p2(n+2,1);
    vp aris;
    rep(i,1,n+2,1) p1[i]=(p1[i-1]*BASE)%MOD1,p2[i]=(p2[i-1]*BASE)%MOD2;
    rep(i,0,m,1){
        int a,b;cin>>a>>b;
        a--;b--;
        aris.pb({a,b});
    ...
    rep(i,0,n,1) {
        for(int j:ady[i]){
            h1[i]=(h1[i]+p1[j+1])%MOD1;
            h2[i]=(h2[i]+p2[j+1])%MOD2;
        }
        owo[h1[i]*MOD1+h2[i]]++;
    }
    int t=0;
    for(auto &[x,y]:owo) t+=(y*(y-1))/2;
    for(auto [a,b]:aris){
        if(sz(ady[a])!=sz(ady[b])) continue;
        int a1=(h1[a]-p1[b+1]+MOD1)%MOD1,a2=(h2[a]-p2[b+1]+MOD2)%MOD2;
        int b1=(h1[b]-p1[a+1]+MOD1)%MOD1,b2=(h2[b]-p2[a+1]+MOD2)%MOD2;
        if(a1==b1&&b2==a2) t++;
    }
    cout<<t<<endl;
}
