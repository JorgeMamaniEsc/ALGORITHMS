    ...
    while(tt--){
        ...
        while(true){
            ...
            while(!q.empty()){
                ...
                q.pop();
                for(int v:ady[u]){
                    if(w[u][v]==0||parent[v]!=-1) continue;
                    q.push(v);
                    parent[v]=u;
                }
            }
        ...
        while(!q.empty()){
            ...
            q.pop();
            for(int v:ady[u]){
                if(w[u][v]<=0||vis[v]) continue;
                q.push(v);
                vis[v]=true;
            }
        }
        vp va;
        for(auto [x,y]:aris) if((vis[x]&&!vis[y])||(!vis[x]&&vis[y])) va.pb({x,y});

        for(auto [x,y]:va) cout<<x+1<<" "<<y+1<<endl;
    }
