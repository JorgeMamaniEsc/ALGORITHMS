...
void mikunakano(){
    ...
    vvi d1(n,vi(m,inf)),d2(n,vi(m,inf));
    deque<pii> q;
    q.push_front({a,b});
    d1[a][b]=0;
    while(!q.empty()){
        ...
        rep(i,0,4,1){
            ...
            if(ny==y-1){
                if(d1[x][y]+1>=d1[nx][ny]) continue;
                d1[nx][ny]=d1[x][y]+1;
                q.pb({nx,ny});
                continue;
            }
            if(d1[x][y]>=d1[nx][ny]) continue;
            d1[nx][ny]=d1[x][y];
            q.push_front({nx,ny});
        }
    }
    ...
    while(!q.empty()){
        ...
        rep(i,0,4,1){
            ...
            if(nx<0||ny<0||nx>=n||ny>=m||ma[nx][ny]=='*') continue;
            if(ny==y+1){
                if(d2[x][y]+1>=d2[nx][ny]) continue;
                d2[nx][ny]=d2[x][y]+1;
                q.pb({nx,ny});
                continue;
            }
            if(d2[x][y]>=d2[nx][ny]) continue;
            d2[nx][ny]=d2[x][y];
            q.push_front({nx,ny});
        }
    }
    int t=0;
    rep(i,0,n,1) rep(j,0,m,1) if(ma[i][j]=='.'&&d1[i][j]<=lx&&d2[i][j]<=ly) t++;
    cout<<t<<endl;
}
