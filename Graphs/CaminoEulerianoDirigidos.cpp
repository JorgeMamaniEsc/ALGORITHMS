...
signed main(){
    ...
    while(tt--){
        ...
        bool ok=false;
        if(entra[n-1]-sale[n-1]!=1) ok=true;
        rep(i,0,n,1) if((i>0&&i<n-1&&gr[i]>0&&entra[i]!=sale[i])||(i==0&&(sale[i]-entra[i]!=1))) ok=true;
        rep(i,0,n,1) if(gr[i]>0&&dsu.find(0)!=dsu.find(i)) ok=true;
        if(ok){
