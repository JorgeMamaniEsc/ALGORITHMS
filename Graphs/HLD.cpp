...
void mikunakano(){
    ...
    auto dfs2=[&](auto&& self,int u,int p,int w)->void{
        head[u]=w;
        id[u]=k++;
        if(heavy[u]!=-1) self(self,heavy[u],u,w);
        for(int v:ady[u]) if(v!=p&&v!=heavy[u]) self(self,v,u,v);
    };
    dfs2(dfs2,0,0,0);
    ...
    auto f=[&](int u,int v)->void{
        while(head[u]!=head[v]){
            ...
            st.set(1,0,n,id[head[u]],id[u],0);
            u=pa[head[u]];
        }
        if(h[u]>h[v])swap(u,v);
        st.set(1,0,n,id[u],id[v],0);
    };
    int q;cin>>q;
    vi ult(n,-1);
    rep(qq,0,q,1){
        ...
        if(op==1){
            st.set(1,0,n,id[u],id[u]+tam[u]-1,1);
            continue;
        }
        cout<<st.qry(1,0,n,id[u],id[u])<<endl;
    }
