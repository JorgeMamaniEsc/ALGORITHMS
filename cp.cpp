...
bool esPal(int l,int r,Hash &hs,Hash &hr,int n){
    return hs.qry(l,r)==hr.qry(n-1-r,n-1-l);
}
...
void mikunakano(){
    ...
    rep(len,1,n+1,1){
        rep(l,0,n,1){
            ...
            int r=l+len-1;
            if(r>=n) break;
            if(!esPal(l,r,h1,h2,n)) continue;
            int mid=(r-l+1)/2;
            if(mid>0)dp[l][r]=min(dp[l][l+mid-1],dp[r-mid+1][r])+1;
            else dp[l][r]=1;
            pre[1]++;
            pre[dp[l][r]+1]--;
        }
    }
    rep(i,1,n+1,1) pre[i]+=pre[i-1];
    rep(i,1,n+1,1) cout<<pre[i]<<" ";
    cout<<endl;
}
