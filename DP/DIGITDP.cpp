...
int cnt(int pos,int mask,bool ok,bool zer,vi &dig,int base){
    if(pos==sz(dig)) return (mask==0&&!zer?1:0);
    int ne=sz(dig)-pos;
    if(dp[base][ne][mask]!=-1&&!ok&&!zer) return dp[base][ne][mask];
    int t=0;
    rep(i,0,(ok?dig[pos]+1:base),1){
        int nmask=mask;
        bool nzer=(zer&&i==0),nok=(ok&&i==dig[pos]);
        if(!nzer) nmask^=(1ll<<i);
        t+=cnt(pos+1,nmask,nok,nzer,dig,base);
    }
    if(!ok&&!zer) dp[base][ne][mask]=t;
    return t;
}
...
signed main(){
    ...
    while(tt--){
        int q;cin>>q;
        rep(i,0,11,1) rep(j,0,65,1) rep(k,0,(1ll<<10),1) dp[i][j][k]=-1;
        while(q--){
            int b,l,r;cin>>b>>l>>r;
            vi v1=cambase(l-1,b),v2=cambase(r,b);
            cout<<cnt(0,0,true,true,v2,b)-cnt(0,0,true,true,v1,b)<<endl;
        }
    }
