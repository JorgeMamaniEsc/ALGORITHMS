...
signed main(){
    ...
    while(tt--){
        ...
        dfs1(dfs1,0,0);
        auto dfs2=[&](auto&& self,int u,int p,int pa)->void{
            int k=sz(ady[u]);
            vi ord,val,pre(k+2,1),suf(k+2,1);
            dp[u]=(dp[u]*(pa+1))%m;
            for(int v:ady[u]){
                if(v==p) continue;
                ord.pb(v);
            ...
            rep(i,1,sz(val)+1,1) pre[i]=(pre[i-1]*val[i-1])%m;
            nrep(i,sz(val)-1,0,1) suf[i]=(suf[i+1]*val[i])%m;
            rep(i,0,sz(val),1){
                int v=ord[i];
