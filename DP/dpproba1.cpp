...
signed main(){
    ...
    while(tt--){
        ...
        vvvd dp(302,vvd(302,vd(302,-1.0)));
        dp[0][0][0]=0.0;

        auto solve=[&](auto&& self,int x,int y,int z) ->ld{
            if(x<0||y<0||z<0) return 0;
            if(dp[x][y][z]!=-1.0) return dp[x][y][z];
            dp[x][y][z]=(n+x*self(self,x-1,y,z)+y*self(self,x+1,y-1,z)+z*self(self,x,y+1,z-1))/(x+y+z);
            return dp[x][y][z];
        };
        cout<<solve(solve,c1,c2,c3)<<endl;
    }
