...
signed main(){
    ...
    while(tt--){
        ...
        vi cost((1ll<<n),0),dp((1ll<<n),0);
        rep(mask,1,(1ll<<n),1){
            vi ord;
            rep(i,0,n,1) if((1ll<<i)&mask) ord.pb(i);
            for(int x:ord) for(int y:ord) if(x<y) cost[mask]+=ma[x][y];
        }
        rep(mask,1,mpv1(3,n),1){
            int x=mask,g1=0,g2=0;
            rep(i,0,n,1){
                int k=x%3;
                x/=3;
                if(k==2) g2|=(1ll<<i),g1|=(1ll<<i);
                else if(k==1) g1|=(1ll<<i);
            }
            dp[g1]=max(dp[g1],dp[g1^g2]+cost[g2]);
        }
