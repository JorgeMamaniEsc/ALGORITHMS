...
struct Matrix{
    ...
    Matrix operator*(const Matrix &o) const{
        ...
        rep(i,0,n,1){
            rep(k,0,n,1){
                ...
                rep(j,0,n,1){
                    ans.ma[i][j]=(ans.ma[i][j]+ma[i][k]*o.ma[k][j])%MOD;
                }
            }
        }
        return ans;
    ...
    void iden(){
        rep(i,0,n,1) rep(j,0,n,1) ma[i][j]=(i==j);
    }
};

Matrix multi(Matrix a,int b){
    Matrix ans(a.n);
    ans.iden();
    while(b>0){
        if(b&1) ans=ans*a;
...
signed main(){
    ...
    while(tt--){
        ...
        vi dig;
        int n=sz(k);
        rep(i,0,n,1) dig.pb(k[i]-'0');
        vvi dp(n+1,vi(d,-1));
        auto f=[&](auto&& self,int i,int j,bool ok)-> int{
            if(i==n){
                return (j==0?1:0);
            }
            int t=0;
            if(ok){
                rep(idx,0,dig[i]+1,1){
                    t+=self(self,i+1,(idx+j)%d,idx==dig[i]);
                    t%=MOD;;
                }
                return t;
            }
            if(dp[i][j]!=-1) return dp[i][j];
            rep(idx,0,10,1) t=(self(self,i+1,(idx+j)%d,false)+t)%MOD;
            dp[i][j]=t;
            return dp[i][j];
        };
        cout<<(f(f,0,0,true)-1+MOD)%MOD<<endl;
    }
